...
export const SPREADSHEETS: ResourceGroup = {
  ...
  endpoints: [
    ...
    },
    {
      ...
      responseExample: JSON.stringify({
        ...
        columns: [
          ...
          { id: 'col-uuid-2', header: 'Notes', data_type: 'long_text' },
          { id: 'col-uuid-3', header: 'Assigned To', data_type: 'user_ref' },
          { id: 'col-uuid-4', header: 'Linked Opportunity', data_type: 'opportunity_ref' },
          { id: 'col-uuid-5', header: 'Due Date', data_type: 'date' },
          { id: 'col-uuid-6', header: 'Complete', data_type: 'boolean' },
          {
            id: 'col-uuid-7',
            header: 'Status',
            data_type: 'dropdown',
    ...
    },
    {
      ...
      params: [
        ...
        },
        {
          ...
          type: 'array',
          required: false,
          description: 'Optional default ordering. Each entry: { column: "<header>", direction: "asc"|"desc" }. Rows display in this order and imports are sorted the same way. Pass null to clear.',
          in: 'body',
        },
        {
          name: 'merge_rules',
          type: 'object',
          required: false,
          description: 'Optional merge-on-insert config. Shape: { group_by: string[], period_start: string, period_end: string, tolerance_days?: number, concat_columns?: { [header]: { separator: string, order_by: string, direction?: "asc"|"desc" } } }. Pass null to clear.',
          in: 'body',
        },
      ],
      requestExample: JSON.stringify({
        ...
        columns: [
          ...
          { header: 'Notes', data_type: 'long_text' },
          { header: 'Assigned To', data_type: 'user_ref' },
          { header: 'Linked Opportunity', data_type: 'opportunity_ref' },
    ...
    },
    {
      method: 'PATCH',
      path: '/spreadsheets/templates/:id',
      description: 'Update a spreadsheet template. columns is a FULL REPLACEMENT -- include all existing columns with their current id to preserve them. To remove a column without losing row data, set archived: true. Changing columns does not modify existing row data. Same data_type values and option shapes as POST.',
      scopes: ['spreadsheets:write'],
      isWrite: true,
      params: [
        { name: 'id', type: 'uuid', required: true, description: 'Template ID', in: 'path' },
        ...
        { name: 'description', type: 'string', required: false, description: 'Template description', in: 'body' },
        {
          ...
          type: 'array',
          required: false,
          description: 'FULL REPLACEMENT column set. Include id on existing columns to update them; omit id to create a new column. Each: { id?: string, header: string, data_type: "text"|"long_text"|"number"|"date"|"boolean"|"dropdown"|"user_ref"|"opportunity_ref", options?: Array<string | { value: string, color?: palette-key }>, archived?: true }.',
          in: 'body',
        },
        {
          name: 'default_sort',
          type: 'array',
          required: false,
          description: 'Default ordering. Array of { column: string, direction: "asc"|"desc" }. Pass null to clear.',
          in: 'body',
        },
        {
          name: 'merge_rules',
          type: 'object',
          required: false,
          description: 'Merge-on-insert config (or null to clear).',
          in: 'body',
        },
