...
export const EMAIL: ResourceGroup = {
  ...
  "pagination": { "limit": 25, "has_more": false, "next_cursor": null }
    ...
    },
    {
      method: 'POST',
      path: '/email/configs',
      description: 'Create an email configuration. Requires from_email, from_name, and staff_email. Shape validation is enforced: from_email must be on @mail.trustpager.com (error code INVALID_FROM_EMAIL otherwise); gmail_sender_alias must already exist as a "Send mail as" address on the connected Gmail account assigned to this workspace (ALIAS_NOT_CONNECTED otherwise). See GET /email/capabilities for available send_as_aliases.',
      scopes: ['email-config:write'],
      isWrite: true,
      params: [
        { name: 'from_email', type: 'string', required: true, description: 'Postmark sender address. MUST end with @mail.trustpager.com (e.g. "support@mail.trustpager.com"). External addresses are rejected with error code INVALID_FROM_EMAIL.', in: 'body' },
        ...
        { name: 'email_provider', type: 'string', required: false, description: 'Rail indicator: "postmark" (TrustPager Mail, default) or "gmail". Automation actions choose the rail at wizard time -- send_custom_email uses Postmark, send_gmail_email uses Gmail.', in: 'body' },
        { name: 'gmail_sender_user_id', type: 'uuid', required: false, description: 'User UUID whose connected Gmail account owns the alias. Required when setting gmail_sender_alias. Error code GMAIL_NOT_CONNECTED if user has no active Gmail connection.', in: 'body' },
        { name: 'gmail_sender_alias', type: 'string', required: false, description: 'Gmail send-as alias for send_gmail_email automations. Must already exist as a "Send mail as" address on the connected Gmail account AND be assigned to this workspace at /settings/email. Error code ALIAS_NOT_CONNECTED if the alias is not found. Error code GMAIL_USER_REQUIRED if gmail_sender_user_id is omitted.', in: 'body' },
      ],
    },
    {
      method: 'PATCH',
      path: '/email/configs/:id',
      description: 'Partial update of an email configuration. Same shape validation as POST: from_email must be @mail.trustpager.com (INVALID_FROM_EMAIL), gmail_sender_alias must be a registered send-as alias for the connected user (ALIAS_NOT_CONNECTED, GMAIL_NOT_CONNECTED, GMAIL_USER_REQUIRED).',
      scopes: ['email-config:write'],
      isWrite: true,
      params: [
        { name: 'id', type: 'uuid', required: true, description: 'Email config ID', in: 'path' },
        { name: 'from_email', type: 'string', required: false, description: 'Postmark sender address. Must end with @mail.trustpager.com. Error code INVALID_FROM_EMAIL if validation fails.', in: 'body' },
        ...
        { name: 'config_name', type: 'string', required: false, description: 'Config display name', in: 'body' },
        { name: 'is_default', type: 'boolean', required: false, description: 'Set as default config', in: 'body' },
        { name: 'email_provider', type: 'string', required: false, description: 'Rail indicator: "postmark" or "gmail"', in: 'body' },
        { name: 'gmail_sender_user_id', type: 'uuid', required: false, description: 'User UUID whose connected Gmail account owns the alias', in: 'body' },
        { name: 'gmail_sender_alias', type: 'string', required: false, description: 'Gmail send-as alias. Must already be registered in the connected Gmail account and assigned to this workspace. Use GET /email/capabilities to see valid aliases.', in: 'body' },
      ],
    },
    {
      method: 'DELETE',
