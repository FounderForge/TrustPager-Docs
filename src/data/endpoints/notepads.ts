  ...
  endpoints: [
    ...
    { method: 'GET', path: '/notepads/:id', description: 'Retrieve a notepad including full content and visibility.', scopes: ['notepads:read'], isWrite: false, params: [{ name: 'id', type: 'uuid', required: true, description: 'Notepad ID', in: 'path' }] },
    {
      ...
      params: [
        ...
        { name: 'folder_id', type: 'uuid', required: false, description: 'Folder UUID (preferred over folder name). Auto-populates the folder string field.', in: 'body' },
        { name: 'folder', type: 'string', required: false, description: 'Folder name (resolved to existing folder or created if missing). Auto-populates folder_id.', in: 'body' },
        { name: 'visibility', type: 'string', required: false, description: 'all_users (default) or restricted', in: 'body' },
        { name: 'is_favorite', type: 'boolean', required: false, description: 'Mark as favourite', in: 'body' },
        { name: 'customer_id', type: 'uuid', required: false, description: 'Company/account UUID. The new notepad is automatically linked to this company -- its ID is appended to the company notepads array. Makes the note appear in the Notepads section of the company page.', in: 'body' },
        { name: 'contact_id', type: 'uuid', required: false, description: 'Contact UUID. The new notepad is automatically linked to this contact -- its ID is appended to the contact notepads array. Makes the note appear in the Notepads section of the contact page.', in: 'body' },
