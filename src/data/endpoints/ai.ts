...
export const AI: ResourceGroup = {
  ...
  endpoints: [
    ...
    { method: 'POST', path: '/ai/generate-image', description: 'Generate an AI image from a text prompt using the NanoBanana model (google:4@1). Returns a CDN image URL, seed, and dimensions. Costs credits. The image is automatically saved to company files. IMPORTANT: Only 11 specific dimension pairs are accepted -- invalid dimensions return a VALIDATION_ERROR with the full list. Default: 1024x1024. Default output format is lossless PNG; pass output_format to override.', scopes: ['ai:use'], isWrite: true, params: [{ name: 'prompt', type: 'string', required: true, description: 'Text description of the image to generate (2-3000 chars). Be descriptive for best results.', in: 'body' }, { name: 'width', type: 'number', required: false, description: 'Image width in pixels. Must be part of a valid pair. Default 1024. Valid pairs: 1024x1024, 1248x832, 832x1248, 1184x864, 864x1184, 896x1152, 1152x896, 768x1344, 1344x768, 1536x672, 672x1536.', in: 'body' }, { name: 'height', type: 'number', required: false, description: 'Image height in pixels. Must be part of a valid pair with width. See width description for all valid pairs. Default 1024.', in: 'body' }, { name: 'seed', type: 'number', required: false, description: 'Random seed for reproducible results. Omit for random.', in: 'body' }, { name: 'output_format', type: 'string', required: false, description: 'Output image format. Values: "PNG" (default, lossless), "WEBP", "JPEG". PNG is recommended when you plan to run optimize_image afterwards (PNG -> WebP variants). WEBP/JPEG are smaller for direct use.', in: 'body' }] },
    {
      ...
      isWrite: true,
      params: [
        { name: 'prompt', type: 'string', required: true, description: 'Text description of the desired output (2-3000 chars). Describe the new setting, style, or lighting around the preserved subject.', in: 'body' },
        { name: 'reference_image_url', type: 'string', required: false, description: 'Single reference image URL (public http(s) URL). Use this OR reference_image_urls -- not both.', in: 'body' },
        { name: 'reference_image_urls', type: 'string[]', required: false, description: 'Array of 1-3 reference image URLs (public http(s) URLs). Use this OR reference_image_url -- not both.', in: 'body' },
        { name: 'width', type: 'number', required: false, description: 'Output width in pixels. Must be part of a valid pair. Default 1024. Valid pairs: 1024x1024, 1248x832, 832x1248, 1184x864, 864x1184, 896x1152, 1152x896, 768x1344, 1344x768, 1536x672, 672x1536.', in: 'body' },
        { name: 'height', type: 'number', required: false, description: 'Output height in pixels. Must be part of a valid pair with width. Default 1024.', in: 'body' },
        { name: 'seed', type: 'number', required: false, description: 'Random seed for reproducible results. Omit for random.', in: 'body' },
        { name: 'output_format', type: 'string', required: false, description: 'Output image format. Values: "PNG" (default, lossless), "WEBP", "JPEG". PNG is recommended when planning to run optimize_image afterwards (PNG -> WebP variants). WEBP/JPEG are smaller for direct use.', in: 'body' },
      ],
