...
export const COMPANY: ResourceGroup = {
  ...
  endpoints: [
    ...
    { method: 'GET', path: '/company/crm-settings', description: 'Get CRM settings -- custom field definitions, lead sources, lost/won reasons, type option lists, transcript settings, feature toggles, notification emails/phones, proposal-builder config (needs_analysis_config), per-field AI fill prompts (ai_fill_prompts), and more. Read before PATCH to preserve existing values.', scopes: ['company:read'], isWrite: false },
    { method: 'PATCH', path: '/company/crm-settings', description: 'Partial update of CRM settings. Only include fields you want to change; unspecified fields are left unchanged. All fields are stored in company_settings (NOT companies.crm_settings which no longer exists).', scopes: ['company:write'], isWrite: true, params: [
      ...
      { name: 'automation_error_notify_emails', type: 'string[]', required: false, description: 'Email addresses notified when an automation action fails. Must be valid email format. Alerts are throttled to one per error signature per hour per company.', in: 'body' },
      { name: 'automation_error_notify_phones', type: 'string[]', required: false, description: 'Phone numbers notified via SMS when an automation action fails. Must be in E.164 format (e.g. +61431377068). Requires an active phone number in the workspace. Alerts are throttled to one per error signature per hour per company.', in: 'body' },
      { name: 'custom_fields', type: 'object', required: false, description: 'Custom field definitions keyed by entity: { deal: [...], account: [...], contact: [...] }. Each field: { id, label, type, options?, show_on_detail, show_in_table, required?, fill_with_ai?, hidden?, conditional_logic? }. Supported types: text, textarea, number, datetime, checkbox, dropdown (requires options[]), url (renders as clickable link with open-in-new-tab icon -- use for Drive folders, portal links, signed agreement URLs).\n\nconditional_logic (optional) -- controls two behaviours on deal fields:\n\n(1) visibility: { mode: "always" | "when" | "never", conditions?: [{ field_id, operator: "is" | "is_not" | "is_set" | "is_empty", value? }] } -- hides or shows the field on the detail view based on the values of other fields. Conditions are ANDed.\n\n(2) default_rules: array of { conditions: [...], action } evaluated top-to-bottom on every deal create or update via API, MCP, form submission, or UI field commit; first matching rule wins. Supported actions:\n- { type: "today_plus", amount: number, unit: "days"|"weeks"|"months", skip_saturday?: boolean, skip_sunday?: boolean } -- writes today + offset as ISO 8601.\n- { type: "field_plus", source_field_id: string, amount: number, unit: "days"|"weeks"|"months", skip_saturday?: boolean, skip_sunday?: boolean } -- writes source datetime field + offset. No-op if source is empty.\n- { type: "constant", value: string|number|boolean } -- writes a fixed value.\n\nRules only write to a field when it is empty or was last set by a rule. Manual edits are never overwritten. Rule-managed status is tracked internally in metadata._rule_managed -- do not set this key manually.', in: 'body' },
      { name: 'needs_analysis_config', type: 'object', required: false, description: 'Proposal Builder / ai_needs_analysis workspace config. Merge-on-blob -- only the keys you pass are changed, the rest are preserved. Fields: target_audience (string), tone_guide (string, brand voice rules), default_instructions (string, master prompt for every run), max_summary_paragraphs (1-10), include_callout (boolean), min_needs (1-20), max_needs (1-20), include_solutions (boolean), min_deliverables (1-10), max_deliverables (1-10), include_closing_strategy (boolean), include_deliverables (boolean), include_recommendations (boolean), product_sort ("price_asc"|"price_desc"|"relevance"). Configurable in Settings > CRM > Needs Analysis.', in: 'body' },
      { name: 'ai_fill_prompts', type: 'object', required: false, description: 'Per-field "Fill with AI" prompt overrides. Shape: { "<field_id>": "instruction string" }. Read-modify-write -- merge with the current object from GET /company/crm-settings to preserve existing prompts. Pass {} to clear all overrides.', in: 'body' },
      { name: 'quick_links', type: 'object[]', required: false, description: 'Workspace-wide Quick Link type definitions. Each entry: { id: string (stable UUID you generate), label: string (e.g. "Google Drive"), icon: string }. Valid icons: link, google-drive, dropbox, notion, slack, docs, folder. These define the Quick Link buttons shown in the right-rail sidebar of opportunity, company, and contact detail pages. Per-record URLs are stored in each entity\'s metadata.quick_links keyed by the type UUID. Pass [] to remove all types.', in: 'body' },
    ] },
    {
      method: 'GET',
