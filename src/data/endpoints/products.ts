...
export const PRODUCTS: ResourceGroup = {
  ...
  endpoints: [
    ...
    { method: 'POST', path: '/products', description: 'Create a product. name is required.', scopes: ['products:write'], isWrite: true, params: [{ name: 'name', type: 'string', required: true, description: 'Product name', in: 'body' }, { name: 'price', type: 'number', required: false, description: 'Unit price', in: 'body' }, { name: 'currency', type: 'string', required: false, description: 'Currency (default: AUD)', in: 'body' }, { name: 'sku', type: 'string', required: false, description: 'SKU code', in: 'body' }, { name: 'work_order_config', type: 'object', required: false, description: 'Product-specific work order field configuration (fields + statuses). Null = use company default.', in: 'body' }] },
    { method: 'PATCH', path: '/products/:id', description: 'Update a product.', scopes: ['products:write'], isWrite: true, params: [{ name: 'id', type: 'uuid', required: true, description: 'Product ID', in: 'path' }, { name: 'work_order_config', type: 'object', required: false, description: 'Product-specific work order field configuration. Set to null to revert to company defaults.', in: 'body' }] },
