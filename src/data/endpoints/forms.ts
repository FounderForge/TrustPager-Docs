...
export const FORMS: ResourceGroup = {
  ...
  endpoints: [
    ...
    { method: 'PATCH', path: '/forms/templates/:id', description: 'Update a form template. Use settings to configure the completion screen, notification addresses, and PDF archive behaviour.', scopes: ['forms:write'], isWrite: true, params: [{ name: 'id', type: 'uuid', required: true, description: 'Template ID', in: 'path' }, { name: 'name', type: 'string', required: false, description: 'Form name', in: 'body' }, { name: 'settings', type: 'object', required: false, description: 'Template settings JSON. Completion keys: completionMessage (string), completionButtonEnabled (boolean), completionButtonText (string), completionButtonUrl (string), completionButtonNewTab (boolean, default true). Notification key: notifyEmails (string[] - email addresses to notify when this form is completed, overrides workspace default). PDF archive keys: archivePdfOnSubmit (boolean, default false - automatically render a PDF of each completed submission and file it in the Documents library), submissionFolder (string, default "Client Forms" - Documents library folder to archive the PDF into), submissionDocumentType (string, default "Other" - document_type tag on the archived PDF row). Merge with existing settings to avoid overwriting stepCount/stepHeaders.', in: 'body' }] },
    ...
    },
    {
      ...
      isWrite: true,
      params: [
        { name: 'id', type: 'uuid', required: true, description: 'Submission ID (must be a completed submission)', in: 'path' },
        { name: 'folder', type: 'string', required: false, description: 'Documents library folder for the PDF. Overrides the template submissionFolder setting. Default: "Client Forms".', in: 'body' },
        { name: 'document_type', type: 'string', required: false, description: 'document_type tag on the resulting Documents row. Common values: "Other", "Invoice", "Proposal", "Agreement", "Report". Default: template submissionDocumentType setting, then "Other".', in: 'body' },
        { name: 'name', type: 'string', required: false, description: 'Filename base for the PDF (without .pdf extension). A timestamp suffix is appended automatically. Default: "<recipient_name> - <template_name>".', in: 'body' },
        { name: 'description', type: 'string', required: false, description: 'Description on the Documents row. Default: "Archived form submission - completed <date>".', in: 'body' },
      ],
      ...
      responseExample: JSON.stringify({
        data: {
          ...
          pages: 2,
          credits_charged: 2,
          attached_to: { table: 'crm_deals', id: 'deal-uuid' },
          attach_warning: null,
        },
        meta: { credits_remaining: 9488 },
      }, null, 2),
