...
export const FORMS: ResourceGroup = {
  ...
  endpoints: [
    ...
    {
      method: 'GET',
      path: '/forms/submissions/export',
      description: 'Bulk export form submissions as xlsx or csv. Returns a binary file download. Fixed columns: Form Name, Submitted At, Status, Recipient Name, Recipient Email, Contact First Name, Contact Last Name, Contact Phone, Contact Landline, Submission ID. Dynamic columns are added for each unique field label found across the filtered submissions. Up to 10,000 rows. Requires forms:read scope.',
      scopes: ['forms:read'],
      isWrite: false,
      params: [
        { name: 'format', type: 'string', required: false, description: 'Output format: "xlsx" (default) or "csv"', in: 'query' },
        { name: 'template_ids', type: 'string', required: false, description: 'Comma-separated list of form template UUIDs to filter by', in: 'query' },
        { name: 'status', type: 'string', required: false, description: 'Filter by submission status (e.g. "pending", "completed", "expired")', in: 'query' },
        { name: 'from', type: 'string', required: false, description: 'ISO 8601 start date filter on created_at (e.g. 2025-01-01T00:00:00Z)', in: 'query' },
        { name: 'to', type: 'string', required: false, description: 'ISO 8601 end date filter on created_at (e.g. 2025-12-31T23:59:59Z)', in: 'query' },
      ],
      requestExample: 'GET /forms/submissions/export?format=xlsx&template_ids=uuid1,uuid2&status=completed&from=2025-01-01T00:00:00Z',
      responseExample: 'Binary xlsx or csv file. Response headers include X-Row-Count with the number of data rows.',
    },
