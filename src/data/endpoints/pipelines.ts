...
export const PIPELINES: ResourceGroup = {
  ...
  endpoints: [
    ...
    { method: 'GET', path: '/pipelines/:id/stages', description: 'List stages for a pipeline, ordered by position. Each stage includes a deal_count field (number of opportunities currently in that stage -- field name preserved for backward compatibility) so Kanban column headers can render counts without a second query.', scopes: ['pipelines:read'], isWrite: false, params: [{ name: 'id', type: 'uuid', required: true, description: 'Pipeline ID', in: 'path' }] },
    ...
    { method: 'GET', path: '/pipelines/:id/deals', description: 'DEPRECATED -- prefer GET /opportunities?pipeline_id=X (legacy alias: GET /deals?pipeline_id=X). The /opportunities endpoint returns the same data plus full pipeline placements on every opportunity and supports additional filters (stage_id, status, search, pagination). This endpoint now also attaches placements for backward compatibility but may be removed in a future version.', scopes: ['pipelines:read', 'opportunities:read'], isWrite: false, params: [{ name: 'id', type: 'uuid', required: true, description: 'Pipeline ID', in: 'path' }] },
    { method: 'GET', path: '/pipelines/:id/summary', description: 'Get pipeline summary with opportunity counts and values per stage.', scopes: ['pipelines:read'], isWrite: false, params: [{ name: 'id', type: 'uuid', required: true, description: 'Pipeline ID', in: 'path' }] },
  ],
};
